...
function SalesSummary({ brandId }: { brandId: string }) {
  ...
  return <><header className="dashboard-header"><div><div className="eyebrow"><span /> 总经理经营工作台 · V1</div><h1>销售全渠道</h1><p>{brand.name} · 2026年7月 · 销售收入</p></div><div className="source-pill"><Database size={16} /> 数据截至 <span>{salesData.actual_cutoff}</span></div></header><section className="table-panel"><div className="table-heading"><div><h2>7月渠道完成情况</h2><p>采用已确认销售口径，点击首页趋势可切换月份。</p></div></div><Table><TableHeader><TableRow><TableHead>渠道</TableHead><TableHead className="text-right">目标</TableHead><TableHead className="text-right">实际</TableHead><TableHead className="text-right">完成率</TableHead><TableHead>数据状态</TableHead></TableRow></TableHeader><TableBody>{rows.map(({channel,point}: any) => <TableRow key={channel.id}><TableCell className="font-semibold">{channel.name}</TableCell><TableCell className="text-right">{point?.target == null ? '—' : currency.format(point.target)}</TableCell><TableCell className="text-right">{point?.actual == null ? '—' : currency.format(point.actual)}</TableCell><TableCell className="text-right">{point?.completion_rate == null ? '—' : percent.format(point.completion_rate)}</TableCell><TableCell>{channel.data_state_name}</TableCell></TableRow>)}</TableBody></Table></section></>;
}
